// ------------------------------------------------------------------------
// Implementation adapted from https://github.com/unageek/Deque
// MIT License
// ------------------------------------------------------------------------
...
namespace Readymade.Machinery.Shared {
    ...
    public sealed class Deque<T> : IList<T>, IReadOnlyList<T>, IList {
        ...
        public int BinarySearch ( int index, int count, T item, IComparer<T>? comparer ) {
            ...
            if ( IsContiguous ( index, count ) ) {
                ...
                if ( read + index < Capacity ) {
                    int i = Array.BinarySearch ( _buf, read + index, count, item, comparer );
                    return i < 0 ? ~( ~i - read ) : i - read;
                } else {
                    int i = Array.BinarySearch ( _buf, read + index - Capacity, count, item, comparer );
                    return i < 0 ? ~( ~i - read + Capacity ) : i - read + Capacity;
                }
            } else {
                int leftLen = Capacity - read;
                int leftCount = leftLen - index;
                int rightCount = count - leftCount;

                int i = Array.BinarySearch ( _buf, read + index, leftCount, item, comparer );
                if ( i >= 0 )
                    return i - read;
                if ( ~i < Capacity )
                    return ~( ~i - read );

                int j = Array.BinarySearch ( _buf, 0, rightCount, item, comparer );
                if ( j >= 0 )
                    return j + leftLen;
        ...
        public Deque<TOutput> ConvertAll<TOutput> ( Converter<T, TOutput> converter ) {
            ...
            Deque<TOutput> deque = new Deque<TOutput> ( count );
        ...
        public int IndexOf ( T item, int index, int count ) {
            ...
            if ( IsContiguous ( index, count ) ) {
                int i = Array.IndexOf ( _buf, item, WrapIndex ( read + index ), count );
                ...
                int i = Array.IndexOf ( _buf, item, read + index, leftCount );
                if ( i != -1 )
                    return i - read;

                int j = Array.IndexOf ( _buf, item, 0, rightCount );
                if ( j != -1 )
                    return j + leftLen;
        ...
        public int LastIndexOf ( T item, int index, int count ) {
            ...
            if ( IsContiguous ( index, count ) ) {
                int i = Array.LastIndexOf ( _buf, item, WrapIndex ( read + ToRightIndex ( index, count ) ), count );
                return i == -1 ? i : WrapIndex ( i - read );
            } else {
                int leftLen = Capacity - read;
                int leftCount = leftLen - index;
                ...
                int j = Array.LastIndexOf ( _buf, item, ToRightIndex ( 0, rightCount ), rightCount );
                if ( j != -1 )
                    return j + leftLen;

                int i = Array.LastIndexOf ( _buf, item, read + ToRightIndex ( index, leftCount ), leftCount );
                if ( i != -1 )
                    return i - read;
        ...
        public void TrimExcess () {
            int newCapacity = NextCapacity ( Count );
